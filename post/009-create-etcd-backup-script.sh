          ...
          containers:
            - name: backup
              ...
              command:
                - "/bin/bash"
                - "-c"
                - oc get no -l node-role.kubernetes.io/master --no-headers -o name | head -n 1 | while read node; do oc debug $node -- bash -c "chroot /host && if [ ! -d /home/core/backup ]; then mkdir -p /home/core/backup; fi && sudo -E  mount -t nfs $etcd_backup_nfs_path  /home/core/backup && sudo -E /usr/local/bin/cluster-backup.sh /home/core/backup && sudo -E find /home/core/backup/ -type f -mtime +4 -delete"; done
          restartPolicy: "Never"
