          ...
          containers:
            - name: backup
              ...
              command:
                - "/bin/bash"
                - "-c"
                - oc get no -l node-role.kubernetes.io/master --no-headers -o name | head -n 1 | while read node; do oc debug $node -- bash -c "chroot /host  sudo -E  mount -t nfs $nfs_backup  /home/core/backup && chroot /host sudo -E /usr/local/bin/cluster-backup.sh /home/core/backup && chroot /host sudo -E find /home/core/backup/ -type f -mmin +1 -delete"; done
          restartPolicy: "Never"
